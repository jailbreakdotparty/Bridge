...
public class Alertinator {
    ...
    func alert(title: String, body: String, showCancel: Bool = true, actionLabel: String = "OK") {
        Task { @MainActor in
            alertController = UIAlertController(title: title, message: body, preferredStyle: .alert)
            if showCancel {
                alertController?.addAction(.init(title: actionLabel, style: .cancel))
            }
    ...
    func alert(title: String, body: String, showCancel: Bool = true, showContinue: Bool = false, continueLabel: String = "Continue", continueAction: @escaping () -> Void = {}, actionLabel: String = "OK", action: @escaping () -> Void) {
        Task { @MainActor in
            alertController = UIAlertController(title: title, message: body, preferredStyle: .alert)
            if showContinue {
